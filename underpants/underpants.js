...
//     for (let i = 0; i < array.length - 1; i++) {
//         var element = array[i];
//         var actionResult = action(element, i, array);
...
*   2) A function
* Objectives:
*   1) call <function> for each element in <array>
...
_.contains = function (array, value) {
    var result;
    (_.indexOf(array, value) !== -1) ? result = true : result = false;
    return result;
    };
...
_.every = function (collection, action) {
  if (action) {
        var mapResult = _.map(collection, action);
        if (_.contains(mapResult, false)) {
            return false;
        } else {
            return true;
        }
  } else if (!action) {
    var mapResult2 = _.map(collection, function(value){ return value;});
        if (_.contains(mapResult2, false)) {
            return false;
        } else {
            return true;
        }
