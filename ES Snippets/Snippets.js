...
// Count occurences
// @param {Number} a number that you are evalating
// @param {Number} target number in question
//
// ex: countOccurences(012322, 2) // returns 3; there are 3 occurences of 2 in the number
...
var countOccurences = function(number, target){
  ...
  for (let i = 0; i < numAsString.length; i++){
      if (target.toString() === numAsString[i]){
        count++;
      }
  } return count;
};
...
// Longest Word
// Takes a string, sanitizes the symbols from it, determines what the longest word was, and returns that word as a string
// @param: {String} sen: a string that can conatin multiple words and/or symbols
//
// Example:
// var sen1 = "fun&!! time";
// LongestWord(sen1) //returns"time"

var sen2 = "I love dogs";
//Output:"love"
...
// LetterCapitalize
// Takes a string and returns each word capitalized

var str = "oh no he has gone off";
...
// Map example
// Example of map funciton with console logging to debug
...
var averageBalance = function(array) {
  ...
  map(array, function(element, index, array) {
    ...
    console.log("allBalancesRaw");
    console.log(allBalancesRaw);
  
  var allBlancesWithoutDollarSign = [];
  
  map(allBalancesRaw, function(element, index, array) {
    allBlancesWithoutDollarSign.push(element.slice(1,2) + element.slice(3));
  });

    console.log("allBlancesWithoutDollarSign");
    console.log(allBlancesWithoutDollarSign);
    
  
  var allBlanacesStringToNumber = [];
  
  map(allBlancesWithoutDollarSign, function(element, index, array) {
    allBlanacesStringToNumber.push(parseFloat(element));
  });
  
  
    console.log("allBlanacesStringToNumber");
    console.log(allBlanacesStringToNumber);
    
  
  var allBalancesTotal = null;
  
    map(allBlanacesStringToNumber, function(element, index, array) {
    allBalancesTotal += element;
  });

    console.log("allBalancesTotal");
    console.log(allBalancesTotal);
    
    
  var avgBalanceAsNumber = (allBalancesTotal/allBalancesRaw.length);
  
    console.log("avgBalanceAsNumber");
    console.log(avgBalanceAsNumber);
    
  var avgBalanceString = avgBalanceAsNumber.toString();
  
    console.log("avgBalanceString");
    console.log(avgBalanceString);
  
  var balanaceObj = {balance: null};
  balanaceObj.balance = avgBalanceString;
  
    console.log("balanaceObj");
    console.log(balanaceObj);
  
  var avgBal = balanaceObj.balance.toString();

    console.log("avgBal");
    console.log(avgBal);
    
    console.log(typeof(avgBal));
...
//     map(array, function(element, index, array){
//         firstLetterArray.push(element.name[0]);
//     });
